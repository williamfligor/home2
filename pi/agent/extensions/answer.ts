...
// Commit:  a3f8ab1  (2026-05-07)
// SPDX-FileCopyrightText: Armin Ronacher
// SPDX-License-Identifier: Apache-2.0
// ----------------------------------------------------------------------------
/**
 * Q&A extraction hook - extracts questions and decisions from assistant responses
 *
 * Custom interactive TUI for responding to questions and making decisions.
 *
 * Demonstrates the "prompt generator" pattern with custom TUI:
 * 1. /answer command gets the last assistant message
 * 2. Shows a spinner while extracting questions and decisions as structured JSON
 * 3. Presents an interactive TUI to navigate and respond
 * 4. Submits the compiled responses when done
 */
...
const SYSTEM_PROMPT = `You are an extractor. Given text from a conversation, extract any questions or decisions that need user input.

Output a JSON object with this structure:
{
  "questions": [
    {
      "question": "The question or decision text",
      "context": "Optional context that helps answer",
...
Rules:
- Extract explicit questions (ending with ?) as type "question"
- Also extract implicit decisions or choices where the assistant presented options, asked for a preference, or needs user input to proceed
- Keep items in the order they appeared
- Be concise with the text
- Include context only when it provides essential information for responding
- If nothing needs user input, return {"questions": []}

Examples of decisions (type "decision"):
- "We could go with Option A or Option B" → implied choice needed
- "Here are the trade-offs between X and Y" → needs user to pick
- "Let me know your preference" → explicit decision request
- "I'll proceed unless you object" → implicit decision
...
Example output:
{
  "questions": [
    ...
    {
      "question": "Choose between REST and GraphQL for the API",
      "context": "REST is simpler but GraphQL gives more flexibility for complex queries",
      "type": "decision"
    }
  ]
}`;
...
class QnAComponent implements Component {
	...
	private submit(): void {
		...
		for (let i = 0; i < this.questions.length; i++) {
			...
			const label = q.type === 'decision' ? 'Decision' : 'Q';
			parts.push(`${label}: ${q.question}`);
			if (q.context) {
	...
	render(width: number): string[] {
		...
		const title = `${this.bold(this.cyan("Questions & Decisions"))} ${this.dim(`(${this.currentIndex + 1}/${this.questions.length})`)}`;
		...
		const prefix = q.type === 'decision' ? this.bold("Decision:") : this.bold("Q:");
		const questionText = `${prefix} ${q.question}`;
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			if (extractionResult.questions.length === 0) {
				ctx.ui.notify("No questions or decisions found in the last message", "info");
				return;
			}
	...
	pi.registerCommand("answer", {
		description: "Extract questions and decisions from last assistant message into interactive responses",
		handler: (_args, ctx) => answerHandler(ctx),
	});

	pi.registerShortcut("ctrl+.", {
		description: "Extract and respond to questions and decisions",
